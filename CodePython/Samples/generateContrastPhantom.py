...
def generateContrastPhantom(dimX, dimY, pixsize, angle):
    ...
    supportRadius = 15  # mm
    TubesCenters = [[22, 7], [16.4, 4.7], [10, 7], [6, 12.5], [6, 19.5], [
        10, 25], [16.4, 27], [22, 25], [21, 16], [11, 16], [16, 11], [16, 21]]
    Nmat = 12
...
def openContrastPhantom(myGeometryFolder, dimX, dimY, pixsize, oversamp, angle):
    ...
    if filepaths:
        ...
        raise Exception(
            "The contrast phantom geometry you are trying to load does not exist or is incorrectly named", filepath)
    return geometry
...
if __name__ == "__main__":
    ...
    magnification = (dist_sample_detector+dist_source_sample) / \
        dist_source_sample
...
#    geometry=geometry.tolist()
    ...
    filepath = "ContrastPhantom/CP_pixsize" + \
        pixsizestr.replace(".", "p")+"um_"+str(dimX)+"x" + \
        str(dimY)+"_angle"+str(angle)+'deg'
