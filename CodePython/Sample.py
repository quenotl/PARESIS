...
class Sample:
    ...
    def defineCorrectValuesSample(self):
        ...
        for currentSample in self.xmldocSample.documentElement.getElementsByTagName("sample"):
            correctSample = self.getText(
                currentSample.getElementsByTagName("name")[0])

            if correctSample == self.myName:
                self.myType = (self.getText(
                    currentSample.getElementsByTagName("myType")[0]))
                self.myMaterials = self.getText(
                    currentSample.getElementsByTagName("myMaterials")[0])
                self.myMaterials = list(self.myMaterials.split(","))
                self.myGeometryFunction = self.getText(
                    currentSample.getElementsByTagName("myGeometryFunction")[0])

                if self.myGeometryFunction == "getMembraneFromFile" or self.myGeometryFunction == "getMembraneSegmentedFromFile":
                    self.myMembraneFile = self.getText(
                        currentSample.getElementsByTagName("myMembraneFile")[0])
                    self.myPMMAThickness = float(self.getText(
                        currentSample.getElementsByTagName("myPMMAThickness")[0]))
                if self.myGeometryFunction == "getMembraneSegmentedFromFile":
                    self.myMeanSphereRadius = float(self.getText(
                        currentSample.getElementsByTagName("myMeanSphereRadius")[0]))
                    self.myNbOfLayers = int(self.getText(
                        currentSample.getElementsByTagName("myNbOfLayers")[0]))

                if self.myGeometryFunction == "get_my_thickness":
                    if self.myName != "air_volume":
                        self.myThickness = float(self.getText(
                            currentSample.getElementsByTagName("myThickness")[0]))
                if self.myGeometryFunction == "getSampleFromFile":
                    self.mySampleFile = self.getText(
                        currentSample.getElementsByTagName("mySampleFile")[0])
                if self.myGeometryFunction == "getVolumesFromFiles":
                    self.myGeometriesFolder = self.getText(
                        currentSample.getElementsByTagName("myGeometriesFolder")[0])
                    self.myNumberOfSlice = int(self.getText(
                        currentSample.getElementsByTagName("myNumberOfSlice")[0]))
                    self.myVoxelSize = float(self.getText(
                        currentSample.getElementsByTagName("myVoxelSize")[0]))
                    self.myVolumesFiles = self.getText(
                        currentSample.getElementsByTagName("myVolumesFiles")[0])
                    self.myVolumesFiles = list(self.myVolumesFiles.split(","))

                if self.myGeometryFunction == "openContrastPhantom":
                    self.myGeometryFolder = self.getText(
                        currentSample.getElementsByTagName("myGeometryFolder")[0])
    ...
    def getDeltaBeta(self, sourceSpectrum):
        ...
        try:
            ...
            if not all(material in df.index.tolist() for material in self.myMaterials):
                warnings.warn(
                    'Not all materials in Materials.csv. Now searching in TablesDeltaBeta.xls')
            for material in self.myMaterials:
                beta = []
                delta = []
                for energy, _ in sourceSpectrum:
                    n = xraylib.Refractive_Index(
    ...
    #                    print("\n\n Sample materials : ",self.myMaterials[imat])
                        if myCell.value == self.myMaterials[imat]:
                            ...
                            for energy, _ in sourceSpectrum:
                                ...
                                if energy*1000 < currentCellValue:  # E is in keV in the spectrum and in eV in the tables delta-beta file
                                    ...
                                    print("No delta beta values under",
                                          currentCellValue, "eV")
                                    continue
                                ...
                                currentCellDelta = float(
                                    sh.cell(row, col+1).value)
                                nextCellDelta = float(
                                    sh.cell(row+1, col+1).value)
                                currentCellBeta = float(
                                    sh.cell(row, col+2).value)
                                nextCellBeta = float(
                                    sh.cell(row+1, col+2).value)
                                deltaInterp = abs(nextCellValue-energy*1e3)/step*currentCellDelta+abs(
                                    currentCellValue-energy*1e3)/step*nextCellDelta
                                betaInterp = abs(nextCellValue-energy*1e3)/step*currentCellBeta+abs(
                                    currentCellValue-energy*1e3)/step*nextCellBeta
                ...
                if a != len(self.myMaterials):
                    raise ValueError(
                        "One or more materials have not been found in delta beta tables")
                return
...
class AnalyticalSample(Sample):
    ...
    def getMyGeometry(self, studyDimensions, studyPixelSize, oversamp, pointNum=0, number_of_positions=0):
        ...
        if self.myType == "sample_of_interest":
            ...
            if self.myGeometryFunction == "CreateSampleCylindre":
                self.myGeometry, self.myRadius = CreateSampleCylindre(
                    self.myName, studyDimensions[0], studyDimensions[1], studyPixelSize)
            ...
            if self.myGeometryFunction == "CreateSampleSpheresInTube":
                self.myGeometry = CreateSampleSpheresInTube(
                    self.myName, studyDimensions[0], studyDimensions[1], studyPixelSize)
                return
            if self.myGeometryFunction == "CreateSampleSphere":
                self.myGeometry = CreateSampleSphere(
                    self.myName, studyDimensions[0], studyDimensions[1], studyPixelSize)
            ...
            if self.myGeometryFunction == "generateContrastPhantom":
                self.myGeometry = generateContrastPhantom(
                    studyDimensions[0], studyDimensions[1], studyPixelSize, angle=30)
                self.myGeometry = np.array(self.myGeometry)
                return
            if self.myGeometryFunction == "openContrastPhantom":
                self.myGeometry = openContrastPhantom(
                    self.myGeometryFolder, studyDimensions[0], studyDimensions[1], studyPixelSize, oversamp, angle=90)
                self.myGeometry = np.array(self.myGeometry)
                return

        if self.myType == "membrane":
            if self.myGeometryFunction == "getMembraneFromFile":
                self.myGeometry.append(getMembraneFromFile(
                    self.myMembraneFile, studyDimensions, studyPixelSize, oversamp, pointNum))
                self.myGeometry.append(
                    np.ones((studyDimensions[0], studyDimensions[1]))*self.myPMMAThickness*1e-6)
                self.myGeometry = np.array(self.myGeometry)
                return
            if self.myGeometryFunction == "getMembraneSegmentedFromFile":
                self.myGeometry.append(getMembraneSegmentedFromFile(
                    self, studyDimensions[0], studyDimensions[1], studyPixelSize, oversamp, pointNum))
                self.myGeometry.append(
                    np.ones((studyDimensions[0], studyDimensions[1]))*self.myPMMAThickness*1e-6)
                self.myGeometry = np.array(self.myGeometry)
                return
        ...
        if self.myGeometryFunction == "get_my_thickness":
            self.myGeometry = []
            self.myGeometry.append(
                np.ones((studyDimensions[0], studyDimensions[1]))*self.myThickness*1e-6)
            self.myGeometry = np.array(self.myGeometry)
            return
    ...
    def setWave(self, incidentWave, energy):
        ...
        if self.myGeometry.ndim != 3:
            raise Exception(
                "Sample Geometry has the wrong nb of dim [material, x, y]")
        k = getk(energy*1e3)
        ...
        for imat in range(len(self.myMaterials)):
            ...
            disturbedWave = np.exp(
                (-1j*k*delta[imat]-k*beta[imat])*self.myGeometry[imat])*disturbedWave
        ...
        for imat in range(len(self.myMaterials)):
            ...
            for energyData, betaEn in self.beta[imat]:
                if energyData == energy:
                    beta[imat] = betaEn
#
            disturbedIntensity = np.exp(-2*k*beta[imat]
                                        * self.myGeometry[imat])*disturbedIntensity
