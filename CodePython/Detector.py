...
import xlrd
# from Sample import Sample TODO: inherit from sample to use getDeltaBeta
...
class Detector:
    ...
    def defineCorrectValuesDetector(self):
        ...
        for currentDetector in self.xmldocDetector.documentElement.getElementsByTagName("detector"):
            correctDetector = self.getText(
                currentDetector.getElementsByTagName("name")[0])
            if correctDetector == self.myName:
                self.myDimensions = self.getMyDimensions(currentDetector)
                self.myPixelSize = float(self.getText(
                    currentDetector.getElementsByTagName("myPixelSize")[0]))
                self.myPSF = float(self.getText(
                    currentDetector.getElementsByTagName("myPSF")[0]))

                for node in currentDetector.childNodes:
                    if node.localName == "myEnergyLimit":
                        self.myEnergyLimit = float(self.getText(
                            currentDetector.getElementsByTagName("myEnergyLimit")[0]))
                    if node.localName == "myBinsThresholds":
                        myBinsThresholdsTmp = self.getText(
                            currentDetector.getElementsByTagName("myBinsThresholds")[0])
                        myBinsThresholdsTmp = list(
                            myBinsThresholdsTmp.split(","))
                        self.myBinsThresholds = sorted(
                            [float(ele) for ele in myBinsThresholdsTmp])
                    if node.localName == "myScintillatorMaterial":
                        self.myScintillatorMaterial = (self.getText(
                            currentDetector.getElementsByTagName("myScintillatorMaterial")[0]))
                        self.myScintillatorThickness = float(self.getText(
                            currentDetector.getElementsByTagName("myScintillatorThickness")[0]))
                return
    ...
    def detection(self, incidentWave, effectiveSourceSize):
        ...
        if effectiveSourceSize:
            sigmaSource = effectiveSourceSize / \
                (2*np.sqrt(2*np.log(2)))  # from FWHM to std dev
            incidentWave = gaussian_filter(
                incidentWave, sigmaSource, mode='wrap')
        intensityBeforeDetection = resize(incidentWave, self.myDimensions)
        seed = int(time.time()*100 % (2**32-1))
        rs = np.random.RandomState(seed)
        if self.myPSF:
            detectedImage = gaussian_filter(
                intensityBeforeDetection, self.myPSF, mode='wrap')
        else:
            detectedImage = intensityBeforeDetection

        detectedImage = rs.poisson((detectedImage))

        detectedImage = detectedImage[self.margins:self.myDimensions[0] -
                                      self.margins, self.margins:self.myDimensions[1]-self.margins]
    ...
    def getMyDimensions(self, node):
        dimX = int(self.getText(
            node.getElementsByTagName("dimX")[0]))+self.margins*2
        dimY = int(self.getText(
            node.getElementsByTagName("dimY")[0]))+self.margins*2
...
#                    print("\n\n Sample materials : ",self.myMaterials[imat])
                if myCell.value == self.myScintillatorMaterial:
                    ...
                    for energy, _ in sourceSpectrum:
                        ...
                        betaInterp = abs(nextCellValue-energy*1e3)/step*currentCellBeta + \
                            abs(currentCellValue-energy*1e3)/step*nextCellBeta
                        self.beta.append((energy, betaInterp))

                    return
            raise ValueError(
                "The scintillator material has not been found in delta beta tables")
...
def resize(imageToResize, size):
    ...
    for x0 in range(sizeX):
        for y0 in range(sizeY):
            resizedImage[x0, y0] = np.sum(imageToResize[int(x0*sampFactor):int(
                x0*sampFactor+sampFactor), int(y0*sampFactor):int(y0*sampFactor+sampFactor)])
